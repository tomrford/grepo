...
#[test]
fn init_reuses_legacy_root_without_creating_modern_root() {
    let root = TestDir::new("init-legacy");
    let workspace = root.path.join("workspace");
    let cache_root = root.path.join("cache");
    let state_root = root.path.join("state");
    fs::create_dir_all(workspace.join("grepo")).unwrap();
    fs::write(workspace.join("grepo/.lock"), "[repos.existing]\n").unwrap();
...
#[test]
fn update_project_lock_reports_already_synchronized_cargo_entry_without_rewriting() {
    let root = TestDir::new("project-lock-up-to-date");
    let workspace = root.path.join("workspace");
    let cache_root = root.path.join("cache");
    let state_root = root.path.join("state");
...
"#,
    ...
    .unwrap();
    let initial_lock = r#"[repos.cargo-lock]
backend = "tarball"
source = "cargo:cargo-lock@11.0.1"
...
"#;
    ...
    let report = run_for_test(
        ...
        state_root,
        "git".into(),
        &["update", "--project-lock", "Cargo.lock"],
    )
    .unwrap();

    assert_eq!(
        report.stdout(),
        &["project-lock: matching entries already synchronized".to_string()]
    );
    assert!(
